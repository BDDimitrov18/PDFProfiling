...
def _query_confirm_boundary(img_n, img_n1, page_n, page_n1, model, processor, config, logger: logging.Logger) -> tuple:
    ...
    prompt = (
        ...
        "Answer false ONLY if page {page_n1} visibly continues the same table, narrative, or "
        "document that page {page_n} belongs to (e.g. a table that flows across both pages, "
        "or a numbered list continuing from page {page_n}).\n\n"
        "Respond ONLY with JSON:\n"
...
def _query_rotation(img, page_num: int, model, processor, config, logger: logging.Logger) -> int:
    ...
    return _query_rotation_impl(img, page_num, model, processor, config, logger,
                                ...
                                corroborate_fn=corroborate_aspect)
...
def detect_boundaries(
    ...
    rotation_corroboration: dict[int, int] = {}  # page_num → corroborate_aspect degrees
    ...
    def detect_rotation(p: int) -> None:
        ...
        if p not in rotation_log:
            ...
            # Record aspect corroboration while the image is still in the buffer.
            _corr = corroborate_aspect(page_buffer[p])
            rotation_corroboration[p] = _corr[1] if _corr[0] else 0
            if rotation != 0:
    ...
    rotation_log.update(smooth_rotation_log(rotation_log, logger, corroboration=rotation_corroboration))
