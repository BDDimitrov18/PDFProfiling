...
def _query_confirm_boundary(img_n, img_n1, page_n, page_n1, model, processor, config, logger: logging.Logger) -> tuple:
    ...
    """
    prompt = (
        f"Do page {page_n} and page {page_n1} clearly belong to DIFFERENT documents?\n\n"
        "Consider:\n"
        "- Different subject matter or purpose\n"
        "- Different issuing authority or organization\n"
        "- Different document type (e.g. a permit vs a technical drawing)\n\n"
        "Answer true only if you are confident they are unrelated documents that should be "
        "filed separately.\n\n"
        "Respond ONLY with JSON:\n"
...
def _query_rotation(img, page_num: int, model, processor, config, logger: logging.Logger) -> int:
    """
    Detect if a page needs rotation to be read correctly.
    Returns degrees to rotate counter-clockwise (0, 90, 180, 270) to correct orientation.
    """
    prompt = (
    ...
    raw = _infer(prompt, [img], model, processor, config, logger, max_tokens=80)
    ...
    if rotation not in (0, 90, 180, 270):
        return 0
    # Only rotate when clearly needed — a wrong rotation corrupts the page
    if rotation != 0 and confidence < 95:
        logger.debug(f"  Rotation {rotation}° suggested for p{page_num} but confidence {confidence:.0f}% < 95% — skipping")
        return 0
...
def detect_boundaries(
    ...
    def ensure_oriented(p: int) -> None:
        ensure(p)
        if p not in rotation_log:
            ...
            if rotation != 0:
                ...
                logger.info(f"  Page {p} rotated {rotation}° CCW to correct orientation")
    ...
    for n in range(1, total_pages):
        ...
        # _sig_on_wrong_page disabled — causes systematic false positives across diverse PDFs.
        # The signal_on_page mechanism already handles the cases this was meant to catch.

        if is_end:
            if not style_match and letterhead_match is False:
                conf = min(1.0, conf + 0.15)
                logger.debug(f"  Style break + different letterhead → conf boosted to {conf:.0%}")
            elif not style_match:
                conf = min(1.0, conf + 0.08)
                logger.debug(f"  Style break → conf boosted to {conf:.0%}")
            elif style_match and style_conf >= 0.80:
                # Same-batch scans always match visually — only a tiny penalty, never suppress
                conf = max(0.0, conf - 0.05)
                logger.debug(f"  Style match — small conf penalty to {conf:.0%}")

            if is_end:
                if layout_prev == layout_curr and layout_prev in ("form", "narrative"):
                    if signal not in STRONG_SIGNOFF_SIGNALS:
                        conf = max(0.0, conf - 0.12)
                        logger.debug(f"  Same layout ({layout_prev}→{layout_curr}) — conf penalized to {conf:.0%}")
                if layout_prev != layout_curr and layout_prev and layout_curr and not style_match:
                    # Layout change only meaningful when styles already differ
                    conf = min(1.0, conf + 0.08)
                    logger.debug(f"  Layout change ({layout_prev}→{layout_curr}) — conf boosted to {conf:.0%}")
        ...
        if is_end and "appendix" in signal:
            if not appendix_chain_active:
                ...
                if not standalone:
                    ...
                    logger.info(f"  Appendix chain started at p{n + 1} — boundaries suppressed until closing signature")
                else:
        ...
        if is_end and signal in {"signature_block", "project_signoff"} and effective_end == n:
            ...
            if not starts_new:
                ...
                # ID-continuity: log-only, zero decision impact
                id_curr = _query_doc_identifier(page_buffer[n], n, model, processor, config, logger)
                id_next = _query_doc_identifier(page_buffer[n + 1], n + 1, model, processor, config, logger)
                same_id = bool(id_curr) and id_curr == id_next
                logger.info(f"  [ID-CONTINUITY log-only] p{n}='{id_curr}' p{n+1}='{id_next}' same={same_id}")
        ...
        if is_end and conf < 0.75 and signal not in ("fresh_letterhead", "header_block_reset"):
